...
for polygon in Q:
    ...
    QKStile = {"id":list(edges),"node":polygon,"type":"Q"}
    for edge in edges:
...
for polygon in K:
    ...
    for i in range(0,len(polygon)):
        ...
        dictofset_add(QKSnodeowners,polygon[i],edge)
        dictofset_add(QKSnodeowners,polygon[i-1],edge)
    QKStile = {"id":list(edges),"node":polygon,"type":"K"}
    for edge in edges:
        dictoflist_append(QKSedgeowners,frozenset(edge),QKStile)
for polygon in S:
    edges = []
    for i in range(0,len(polygon)):
        ...
        dictofset_add(QKSnodeowners,polygon[i],edge)
        dictofset_add(QKSnodeowners,polygon[i-1],edge)
    QKStile = {"id":list(edges),"node":polygon,"type":"S"}
    for edge in edges:
        dictoflist_append(QKSedgeowners,frozenset(edge),QKStile)

for edge in QKSedgeowners:
    print edge, QKSedgeowners[edge]
#これでやっとタイルができた。次は、PBCに適した長方形を切りだす。別のプログラムにするか。
